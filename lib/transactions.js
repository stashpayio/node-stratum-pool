...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    for (var i = 0; i < recipients.length; i++){
        ...
        if (recipientReward == 0)
            throw new Error("ERROR: Recipient amount should be non-zero - check the config file");
...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree){
    ...
    /* stash legacy shielded tx */
    if  (rpcData.shielded_tx_enabled !== undefined && txVersion < 3) {
        var p2 = Buffer.concat([
            p2,
            util.varIntBuffer(joinsplits.length) // new Buffer([0])
        ]);
    }
